...
class PlotConfig:
    ...
    topk_fallback_when_empty: bool = False
    topk_threshold_points: int = 101
...
class RunResult:
    ...
    topk_thresholds: np.ndarray
    topk_policy_tp_by_threshold: np.ndarray
    topk_policy_selected_by_threshold: np.ndarray
    saw_probs: bool
...
def _analyze_run(run: RunConfig, settings: PlotConfig, eta_edges: np.ndarray, r_edges: np.ndarray, score_edges: np.ndarray) -> RunResult:
    ...
    topk_thresholds = np.linspace(0.0, 1.0, settings.topk_threshold_points, dtype=np.float64)
    n_thr = int(topk_thresholds.size)
    topk_policy_tp_by_threshold = np.zeros((n_k, n_thr), dtype=np.float64)
    topk_policy_selected_by_threshold = np.zeros((n_k, n_thr), dtype=np.float64)
        ...
        for i, key in enumerate(event_keys):
            ...
            if prob is not None:
                ...
                if n_k > 0:
                    ...
                    # Threshold-first scan with top-k cutoff under the active policy.
                    for it, threshold in enumerate(topk_thresholds):
                        for ik, k in enumerate(topk_limits):
                            if settings.topk_policy == "eta_r_topk":
                                policy_selected = _build_eta_r_topk_mask(
                                    prob,
                                ...
                                policy_selected = _build_global_topk_mask(
                                    prob,
                                    int(k),
                                    min_prob=float(threshold),
                                    fallback_when_empty=settings.topk_fallback_when_empty,
                            ...
                            tp_thr = float(np.sum(policy_selected & truth))
                            selected_thr = float(np.sum(policy_selected))
                            topk_policy_tp_by_threshold[ik, it] += tp_thr
                            topk_policy_selected_by_threshold[ik, it] += selected_thr
    ...
    return RunResult(
        ...
        topk_policy_tp_by_threshold=topk_policy_tp_by_threshold,
        topk_policy_selected_by_threshold=topk_policy_selected_by_threshold,
        saw_probs=saw_probs,
    )
...
def _save_topk_threshold_scan_plot(out_dir: Path, run_name: str, result: RunResult) -> None:
    if result.topk_limits.size == 0 or result.topk_thresholds.size == 0:
        return

    thresholds = result.topk_thresholds
    true_total = result.topk_true[:, None]
    tp = result.topk_policy_tp_by_threshold
    selected = result.topk_policy_selected_by_threshold
    recall = _safe_divide(tp, true_total)
    precision = _safe_divide(tp, selected)

    fig, (ax_p, ax_r) = plt.subplots(2, 1, figsize=(10, 8), sharex=True, constrained_layout=True)
    for i, k in enumerate(result.topk_limits.tolist()):
        ax_p.plot(thresholds, precision[i], label=f"k={k}")
        ax_r.plot(thresholds, recall[i], label=f"k={k}")

    ax_p.set_ylabel("precision")
    ax_p.set_ylim(0.0, 1.0)
    ax_p.grid(alpha=0.25, linestyle="--")
    ax_p.legend(title="top-k", ncol=2)
    ax_p.set_title(f"{run_name}: top-k precision/recall vs threshold")

    ax_r.set_xlabel("threshold on P(is_first)")
    ax_r.set_ylabel("recall")
    ax_r.set_ylim(0.0, 1.0)
    ax_r.grid(alpha=0.25, linestyle="--")

    fig.savefig(out_dir / "first_hit_topk_threshold_scan.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    settings = PlotConfig(
        runs=[
            RunConfig(name="run_a", eval_h5=Path("/share/rcif2/pduckett/hepattn-dq/src/hepattn/experiments/trackml/logs/first-hit-pred_20260227-T124450/ckpts/epoch=041-val_loss=0.03215_test_eval.h5"), color="tab:blue"),
        ],
        ...
        r_range=(0.0, 1.5),
        score_bins=200,
        topk_limits=[600, 1200, 2200, 2400, 2600, 2800],
        ...
        topk_fallback_when_empty=False,  # strict thresholding: if none pass threshold, select none
        topk_threshold_points=101,
    ...
    for run in settings.runs:
        ...
        if settings.write_per_run_plots:
            ...
            if result.topk_limits.size > 0 and result.saw_probs:
                ...
                _save_topk_threshold_scan_plot(run_dir, run.name, result)
