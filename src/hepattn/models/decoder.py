...
class MaskFormerDecoder(nn.Module):
    def __init__(
        ...
        bidirectional_ca_use_q_before_ca: bool = False,
        ...
        self.bidirectional_ca_use_q_before_ca = bidirectional_ca_use_q_before_ca
    ...
    def forward(self, x: dict[str, Tensor], input_names: list[str]) -> tuple[dict[str, Tensor], dict[str, dict]]:
        ...
        for layer_index, decoder_layer in enumerate(self.decoder_layers):
            ...
            if do_diagnostic_logging:
                ...
                q_before_ca = x["query_embed"]
                q_pe = q_before_ca if query_posenc is None else q_before_ca + decoder_layer.scale_pe * query_posenc
                ...
                # Log forward CA attention weights (single head, already on CPU)
                ...
                if decoder_layer.q_ca.fn.last_attn_weights is not None:
                    # Shape is (B, Q, K) for single head - squeeze batch if B=1
                    raw_weights = decoder_layer.q_ca.fn.last_attn_weights
                    if raw_weights.dim() == 3 and raw_weights.shape[0] == 1:
                        fwd_attn_weights = raw_weights.squeeze(0)  # (Q, K)
                    else:
                ...
                if do_bidirectional:
                    ...
                    if self.bidirectional_ca_use_q_before_ca:
                        q_for_bidi = q_before_ca
                    elif self.bidirectional_ca_use_q_before_sa:
                        q_for_bidi = q_after_ca
                    else:
                        q_for_bidi = q_after_sa
                    ...
                    # Log bidirectional CA attention weights (single head, already on CPU)
                    print(
                    ...
                    if decoder_layer.kv_ca.fn.last_attn_weights is not None:
                        # Shape is (B, K, Q) for single head - squeeze batch if B=1
                        raw_weights = decoder_layer.kv_ca.fn.last_attn_weights
                        if raw_weights.dim() == 3 and raw_weights.shape[0] == 1:
                            bidi_attn_weights = raw_weights.squeeze(0)  # (K, Q)
                        else:
                ...
                x["query_embed"], x["key_embed"] = decoder_layer(
                    ...
                    use_q_before_ca=self.bidirectional_ca_use_q_before_ca,
                )
...
class MaskFormerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        q_before_ca = q
        q_pe = q_before_ca if query_posenc is None else q_before_ca + self.scale_pe * query_posenc
        ...
        if do_bidirectional and self.bidirectional_ca:  # self.bidirectional_ca ensures kv_ca exists
            ...
            if use_q_before_ca:
                q_for_bidi = q_before_ca
            elif use_q_before_sa:
                q_for_bidi = q_before_sa
            else:
                q_for_bidi = q
