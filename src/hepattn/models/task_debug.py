...
class ObjectValidTask(Task):
    ...
    def cost(self, outputs, targets):
        ...
        for cost_fn, cost_weight in self.costs.items():
            ...
            # First ensure the target tensor has the right shape
            target_valid = targets[self.target_object + "_valid"]
            if target_valid.dim() == 2:  # [batch, particles]
                target_valid = target_valid.unsqueeze(-1)  # [batch, particles, 1]
            
            # Expand to match the costs tensor shape
            target_valid = target_valid.expand_as(costs[cost_fn])
            costs[cost_fn][~target_valid] = 1e6
        return costs
...
class FPRegressionTaskHitObjRepr(RegressionTask):
    ...
    def forward(self, x: dict[str, Tensor], track_hit_valid_mask: dict[str, Tensor] | None = None, track_valid_mask: dict[str, Tensor] | None = None) -> dict[str, Tensor]:
        ...
        def get_hit_embeds_from_logits(track_hit_assignment_logits, hit_embed):
            ...
            # For visualization: store hit groups and scores for a random batch only
            should_plot = (self.step % self.plot_frequency == 0)
            hit_groups = []
            hit_scores = []
            
            # Select random batch index if we're going to plot
            plot_batch_idx = torch.randint(0, batch_size, (1,)).item() if should_plot else -1
            ...
            for b in range(batch_size):
                for q in range(num_queries):
                    if num_hits_per_track[b, q] > 0:
                        ...
                        # Store for visualization only for randomly selected batch
                        if should_plot and b == plot_batch_idx:
            ...
            # Plot hits for randomly selected batch only and only at specified frequency
            if should_plot and len(hit_groups) > 0:
                self.plot_hit_groups(plot_batch_idx, hit_embed[plot_batch_idx], hit_groups, hit_scores)
                self.step += 1
...
class FPRegressionTaskHitObjCNN(RegressionTask):
    ...
    def __init__(
        ...
        plot_dir: str = "hit_plots",
        plot_frequency: int = 100,  # Plot every N steps
        **kwargs
    ):
        """Regression task that uses hit object representations.
        ...
        combine_phi : bool
            Whether to combine phi coordinates in the hit representation
        plot_dir : str
            Directory to save hit visualization plots
        plot_frequency : int
            How often to plot (every N steps)
        """
        super().__init__(
            name=name,
            target_labels=target_labels,
    ...
    def plot_hit_groups(self, batch_idx: int, hit_coords: torch.Tensor, hit_groups: list[torch.Tensor], scores: list[torch.Tensor]):
        ...
        for group_idx, (group_indices, group_scores) in enumerate(zip(hit_groups, scores)):
            ...
            # Convert scores to float32 before sigmoid and then to numpy
            ...
            for b in range(batch_size):
                for q in range(num_queries):
                    if num_hits_per_track[b, q] > 0:
                        ...
                        # Gather coordinates for selected hits
                        track_hits = input_coords[b, hit_indices]  # [num_selected, coord_dim]
